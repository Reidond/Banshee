...
fn rows() -> Vec<Row> {
    ...
    vec![
        ...
        m("a_flag_disambiguate", DISAMBIGUATE, evt(Key::Char('a'), NONE, "a"), b"a"),
        // flag 8 (report-all-keys): every key becomes CSI u. 'a' == 97.
        m("a_flag_all_keys", REPORT_ALL_KEYS, evt(Key::Char('a'), NONE, "a"), b"\x1b[97u"),
        ...
        m("a_flag_text_only_legacy", REPORT_TEXT, evt(Key::Char('a'), NONE, "a"), b"a"),

        // ============================================================
        // ctrl+a  — C0 collision, the protocol's raison d'être
        // ============================================================
        m("ctrl_a_legacy", 0, ev(Key::Char('a'), CTRL), b"\x01"),
        m("ctrl_a_disambiguate", DISAMBIGUATE, ev(Key::Char('a'), CTRL), b"\x1b[97;5u"),
        m("ctrl_a_all_keys", REPORT_ALL_KEYS, ev(Key::Char('a'), CTRL), b"\x1b[97;5u"),

        // ============================================================
        // ctrl+i vs Tab disambiguation (ctrl+i == 0x09 == Tab in legacy)
        // ============================================================
        m("ctrl_i_legacy_is_tab_byte", 0, ev(Key::Char('i'), CTRL), b"\x09"),
        m("ctrl_i_disambiguate", DISAMBIGUATE, ev(Key::Char('i'), CTRL), b"\x1b[105;5u"),
        // Tab key itself under disambiguation: not ambiguous, no mods → legacy.
        m("tab_disambiguate_stays_legacy", DISAMBIGUATE, ev(Key::Tab, NONE), b"\t"),
        // Tab under report-all-keys → CSI 9 u.
        m("tab_all_keys", REPORT_ALL_KEYS, ev(Key::Tab, NONE), b"\x1b[9u"),
        // shift+Tab under disambiguation: has mods → CSI 9;2 u.
        m("shift_tab_disambiguate", DISAMBIGUATE, ev(Key::Tab, SHIFT), b"\x1b[9;2u"),
        // ctrl+m vs Enter (ctrl+m == 0x0d == Enter).
        m("ctrl_m_disambiguate", DISAMBIGUATE, ev(Key::Char('m'), CTRL), b"\x1b[109;5u"),
        // ctrl+[ vs Escape (ctrl+[ == 0x1b).
        m("ctrl_bracket_disambiguate", DISAMBIGUATE, ev(Key::Char('['), CTRL), b"\x1b[91;5u"),
        ...
        // ============================================================
        m("escape_disambiguate", DISAMBIGUATE, ev(Key::Escape, NONE), b"\x1b[27u"),
        m("escape_all_keys", REPORT_ALL_KEYS, ev(Key::Escape, NONE), b"\x1b[27u"),
        // Escape with a modifier under disambiguation.
        m("shift_escape_disambiguate", DISAMBIGUATE, ev(Key::Escape, SHIFT), b"\x1b[27;2u"),

        // ============================================================
        // Enter / Backspace under flag 8 (report all keys)
        // ============================================================
        m("enter_all_keys", REPORT_ALL_KEYS, ev(Key::Enter, NONE), b"\x1b[13u"),
        m("backspace_all_keys", REPORT_ALL_KEYS, ev(Key::Backspace, NONE), b"\x1b[127u"),
        // Enter/Backspace under disambiguation only (no mods): stay legacy.
        m("enter_disambiguate_stays_legacy", DISAMBIGUATE, ev(Key::Enter, NONE), b"\r"),
        m("backspace_disambiguate_stays_legacy", DISAMBIGUATE, ev(Key::Backspace, NONE), b"\x7f"),
        // ctrl+Enter under disambiguation: has mods → CSI 13;5 u.
        m("ctrl_enter_disambiguate", DISAMBIGUATE, ev(Key::Enter, CTRL), b"\x1b[13;5u"),

        // ============================================================
        // Arrow keys with modifiers (Kitty CSI 1;<m><letter> forms)
        // ============================================================
        m("up_disambiguate_nomods", DISAMBIGUATE, ev(Key::Up, NONE), b"\x1b[1A"),
        m("up_shift_disambiguate", DISAMBIGUATE, ev(Key::Up, SHIFT), b"\x1b[1;2A"),
        m("up_ctrl_disambiguate", DISAMBIGUATE, ev(Key::Up, CTRL), b"\x1b[1;5A"),
        m("down_alt_disambiguate", DISAMBIGUATE, ev(Key::Down, ALT), b"\x1b[1;3B"),
        m("right_ctrl_shift_disambiguate", DISAMBIGUATE, ev(Key::Right, CTRL | SHIFT), b"\x1b[1;6C"),
        m("left_disambiguate", DISAMBIGUATE, ev(Key::Left, NONE), b"\x1b[1D"),
        m("home_disambiguate", DISAMBIGUATE, ev(Key::Home, NONE), b"\x1b[1H"),
        m("end_ctrl_disambiguate", DISAMBIGUATE, ev(Key::End, CTRL), b"\x1b[1;5F"),
        // Arrow keys retain the Kitty form even under report-all-keys.
        m("up_all_keys", REPORT_ALL_KEYS, ev(Key::Up, NONE), b"\x1b[1A"),

        // ============================================================
        // Navigation tilde keys under flags
        // ============================================================
        m("pageup_disambiguate", DISAMBIGUATE, ev(Key::PageUp, NONE), b"\x1b[5~"),
        m("pagedown_shift_disambiguate", DISAMBIGUATE, ev(Key::PageDown, SHIFT), b"\x1b[6;2~"),
        m("insert_disambiguate", DISAMBIGUATE, ev(Key::Insert, NONE), b"\x1b[2~"),
        m("delete_ctrl_disambiguate", DISAMBIGUATE, ev(Key::Delete, CTRL), b"\x1b[3;5~"),

        // ============================================================
        // Function keys under flags
        // ============================================================
        m("f1_disambiguate", DISAMBIGUATE, ev(Key::F(1), NONE), b"\x1b[1P"),
        m("f4_shift_disambiguate", DISAMBIGUATE, ev(Key::F(4), SHIFT), b"\x1b[1;2S"),
        m("f5_disambiguate", DISAMBIGUATE, ev(Key::F(5), NONE), b"\x1b[15~"),
        m("f12_ctrl_disambiguate", DISAMBIGUATE, ev(Key::F(12), CTRL), b"\x1b[24;5~"),
        ...
        // ============================================================
        m("alt_a_disambiguate", DISAMBIGUATE, evt(Key::Char('a'), ALT, "a"), b"\x1b[97;3u"),
        // Alt under legacy stays ESC-prefixed meta.
        m("alt_b_legacy", 0, evt(Key::Char('b'), ALT, "b"), b"\x1b\x62"),
        ...
        m("altgr_at_disambiguate", DISAMBIGUATE, evt(Key::Char('q'), ALT_GR, "@"), b"@"),
        m("altgr_at_all_keys", REPORT_ALL_KEYS, evt(Key::Char('q'), ALT_GR, "@"), b"@"),
        m(
            "altgr_at_all_flags",
        ...
        m("deadkey_composed_e_circumflex_legacy", 0, evt(Key::Char('e'), NONE, "\u{EA}"), "\u{EA}".as_bytes()),
        // Bare dead-key press (no committed text) → no bytes.
        m("deadkey_bare_circumflex_noop", 0, ev(Key::Char('^'), NONE), b""),
        m("deadkey_bare_circumflex_noop_kitty", DISAMBIGUATE, ev(Key::Char('^'), NONE), b""),
        // ctrl+space → NUL legacy; CSI u under kitty (space == 32).
        m("ctrl_space_legacy_nul", 0, ev(Key::Char(' '), CTRL), b"\x00"),
        m("ctrl_space_disambiguate", DISAMBIGUATE, ev(Key::Char(' '), CTRL), b"\x1b[32;5u"),
        // ctrl+[ → ESC legacy.
        m("ctrl_bracket_legacy_esc", 0, ev(Key::Char('['), CTRL), b"\x1b"),
        // numpad Enter is folded into Key::Enter.
        m("numpad_enter_legacy", 0, ev(Key::Enter, NONE), b"\r"),
        m("numpad_enter_all_keys", REPORT_ALL_KEYS, ev(Key::Enter, NONE), b"\x1b[13u"),
...
fn kitty_golden_matrix() {
    let rows = rows();
    assert!(rows.len() >= 40, "expected >= 40 golden cases, got {}", rows.len());
