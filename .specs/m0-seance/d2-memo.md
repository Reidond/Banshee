...
| Keyboard focus + TSF | **Reactor exposes NO key/char/focus/IME API** at the pinned rev: `KeyDown`/`KeyUp`/`CharacterReceived`/`GotFocus`/`LostFocus` are unimplemented vtable stubs (`bindings.rs`); no TSF/CoreText surface exists. **Second finding (first interactive run): WinUI 3 delivers keyboard input to its InputSite child HWND — a top-level subclass sees no WM_CHAR.** The probe is now thread-scoped hooks (WH_GETMESSAGE + WH_CALLWNDPROC), verified with real foreground keystrokes: KEY/CHAR/FOCUS all captured and forwarded to the PTY (Gherkin "plain typing" scenario: input half PASS, automated). **Design consequence for M1: the terminal input path (SPEC §6.3, NFR-5) is shell-owned Win32 message hooks + TSF regardless of tier.** **Gherkin "AltGr is not misread as Ctrl+Alt": PASS (automated, 2026-07-04)** — Ukrainian layout, synthesized hardware-level input: AltGr+г delivered `ґ` (U+0491) as a clean WM_CHAR forwarded to the PTY as UTF-8; plain г → U+0433; contrast Ctrl+LAlt on an unmapped key produced KEY events but zero CHAR and nothing to the PTY (evidence: target/altgr-ua.log run). Remaining `PENDING-MANUAL`: JA IME commit-once, focus-loss-mid-composition cancel (both need a live IME) — see MANUAL-MATRIX.md. |
