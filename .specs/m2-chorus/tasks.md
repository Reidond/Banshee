...
- **Group C — Text pipeline completion** (parallel to A/B):
  - C1 (M): ligatures (HarfBuzz features, config-off switch)
  - C2 (M): emoji (COLR/bitmap → RGBA atlas page)
  - C3 (M): custom box-drawing/block/Powerline rasterizer (seam-free)
...
- **Group E — Themes & config** (parallel):
  ...
  - E2 (S): keybind list form + conflict detection; `conf.d/*.toml` merge
