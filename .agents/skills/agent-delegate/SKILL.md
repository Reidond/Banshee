...
`.Codex/skills/fable-orchestrate/SKILL.md` and say which you chose.
...
2. **Brief.** Write the brief per the contract — one concern, inlined context, self-checkable
   ...
   `.Codex/skills/fable-orchestrate/references/brief-template.md`. If the task needs decisions
...
3. **Mode.** Opinion/analysis (`--second-opinion`, reviews, diagnosis) → read-only mode.
   ...
   `.Codex/skills/fable-orchestrate/references/external-agents.md`. Codex plugin installed →
