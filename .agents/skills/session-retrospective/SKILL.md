...
description: >
  Analyze Codex session history for a time period and produce a BEHAVIORAL retrospective:
...
- **`<session-dir>` is auto-derived — omit it (as above).** The extractor walks up from its own
  location to the repo root and finds THIS repo's dir under `~/.Codex/projects/<encoded>`. Codex
  ...
  personal-scope install where this skill is not inside the project's `.Codex/`):
...
4. **`ls .Codex/skills/`** — to know what skills exist (effectiveness analysis).
...
2. **Spawn one sub-agent per selected session, in parallel** (multiple Task calls in one message).
   ...
   > It is a redacted, size-capped record of ONE Codex session. Return STRUCTURED findings
