...
name: skill-creator
description: >
  Interactive guide for creating new Codex skills. Walks through use case definition,
  frontmatter generation, instruction writing, supporting file creation, and validation.
  Enforces the Agent Skills open standard and Codex extensions: progressive disclosure,
  proper invocation control, kebab-case naming. Use when the user says "create a skill",
  "new skill", "build a skill", or "I want to teach Codex to..."
...
You are a Skill Architect helping the user create a well-structured Codex skill.
You know the Agent Skills open standard (agentskills.io/specification), Codex's
skill extensions (code.Codex.com/docs/en/skills), and the patterns that make skills
...
- User says "create a skill", "new skill", "build a skill", "teach Codex to..."
...
2. **Who invokes it?**
   ...
   - Codex-only (background knowledge like conventions, style guides)
...
5. **Where should it live?**
   - Project: `.Codex/skills/` (this project only)
   - Personal: `~/.Codex/skills/` (all your projects)
   - Plugin: `<plugin>/skills/` (where plugin is enabled, uses `plugin-name:skill-name` namespace)
   - Nested: `packages/<pkg>/.Codex/skills/` (auto-discovered in monorepos when editing files in that subtree)
...
**Content type** (from Codex docs):

| Type | Pattern | Key Techniques |
|------|---------|----------------|
| **Reference content** | Background knowledge Codex applies contextually | Conventions, patterns, domain expertise |
...
| **Reference** | Background knowledge loaded automatically | Codex only |
...
name: {kebab-case-name}              # Spec: required. Codex: optional (falls back to
...
description: >                        # Spec: required. Codex: recommended (falls back
...
user-invocable: {true|false}           # false → hidden from / menu (Codex can still invoke
...
hooks: {...}                           # Hooks scoped to skill lifecycle events
                                       # (see code.Codex.com/docs/en/hooks)
---
```

**Important invocation nuances** (from Codex docs):
- `user-invocable: false` only controls `/` menu visibility, NOT Skill tool access.
  This is the correct setting for internal pipeline skills — Codex can still invoke them.
...
Is it only called by other skills? (internal pipeline step)
  YES → user-invocable: false  (hides from menu; Codex can still invoke via Skill tool)
...
#### String substitutions (Codex feature)
...
#### Extended thinking (Codex feature)
...
#### Dynamic context injection (Codex feature)

The `!`command`` syntax runs shell commands before the skill content is sent to Codex:
...
The command output replaces the placeholder — Codex only sees the result, not the command.
...
1. **Translate research findings into actionable instructions** — don't just copy
   the brief; transform knowledge into guidance Codex can follow
...
- Reference supporting files explicitly so Codex knows when to load them
...
- [ ] `name` field matches folder name, 1-64 chars (spec: required; Codex: optional)
...
- [ ] `allowed-tools` follows principle of least privilege — but note it is **experimental**: Codex's SKILL.md parser may not recognize it (an IDE diagnostic can flag it as unsupported), so treat it as informational, not a load-bearing tool restriction
- [ ] No name collision with skills at higher precedence scopes

**Composability:**
- [ ] Works alongside existing skills without conflict
- [ ] Doesn't duplicate content from AGENTS.md or AGENTS.md
...
1. If it's a project skill, add it to the project's `AGENTS.md` skills listing
2. **Append a changelog entry** — invoke the `ai-changelog` skill (read
   `.Codex/skills/ai-changelog/SKILL.md`) with a `SKILL-ADDED` entry containing:
   the skill name, type, files created, and which workflows are affected
3. **Record improvement hypothesis** — read `.Codex/skills/ai-improvement-tracker/SKILL.md`
   and evaluate whether the changelog entry warrants a testable hypothesis about expected value
4. Test triggering: ask "What skills are available?" to verify Codex sees it
...
**Location:** .Codex/skills/{name}/
**Type:** {Task | Reference | Review | Workflow}
**Invocation:** {User-only | Codex-only | Both | Internal-only}
